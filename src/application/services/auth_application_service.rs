...
impl AuthApplicationService {
    ...
    pub async fn register(&self, dto: RegisterDto) -> Result<UserDto, DomainError> {
                    ...
                    if admin_count > 0 {
                        ...
                        match self.count_all_users().await {
                            Ok(user_count) => {
                                ...
                                tracing::info!(
                                    "Permitiendo creación de admin en instalación limpia"
                                );
                            }
                            Err(e) => {
        ...
        // Crear usuario
        let user =
            User::new(dto.username.clone(), dto.email, dto.password, role, quota).map_err(|e| {
                DomainError::new(
    ...
    pub async fn login(&self, dto: LoginDto) -> Result<AuthResponseDto, DomainError> {
        ...
        let mut user = self
            ...
            .await
            .map_err(|_| {
                tracing::error!("Error al buscar usuario: {}", dto.username);
                DomainError::new(
                    ErrorKind::AccessDenied,
                    "Auth",
                    "Credenciales inválidas. Username not found",
                )
            })?;
        ...
        tracing::info!("Usuario encontrado: {:?}", user);

        // Verificar contraseña
        let is_valid = user.verify_password(&dto.password).map_err(|_| {
    ...
    pub async fn change_password(
        ...
        let is_valid = user.verify_password(&dto.current_password).map_err(|_| {
            DomainError::new(
                ErrorKind::AccessDenied,
                "Auth",
