...
#[utoipa::path(
        post,
        path = "/files/upload",
        request_body(content = FileUploadDto, content_type = "multipart/form-data"),
        responses(
            (status = 201, description = "File uploaded successfully", body = FileDto),
...
#[utoipa::path(
        ...
        params(
            ...
            ("compress" = Option<bool>, Query, description = "Force compression (true/false)"),
            ("compression_level" = Option<String>, Query, description = "Compression level (none, fast, best, default)"),
            ("inline" = Option<bool>, Query, description = "Force inline display (true/false)")
        ),
        responses(
                ...
                match service.get_file_content(&id).await {
                    Ok(content) => {
                        ...
                        if should_compress {
                            ...
                            headers
                                .insert(header::CONTENT_ENCODING.to_string(), "gzip".to_string());
                            headers
                                .insert(header::CONTENT_TYPE.to_string(), file.mime_type.clone());
...
#[utoipa::path(
        ...
        path = "/files",
        params(
            ("folder_id" = Option<String>, Query, description = "Folder ID to list files from")
        ),
        responses(
...
pub async fn list_files(
    ...
    let folder_id = params.get("folder_id").map(|s| s.as_str());
...
#[utoipa::path(
        ...
        request_body = MoveFilePayload,
        responses(
            (status = 200, description = "File moved", body = FileDto),
            (status = 404, description = "File not found"),
