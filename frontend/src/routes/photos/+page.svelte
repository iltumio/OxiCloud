<script lang="ts">
	...
	const JUSTIFIED_GAP = 8; // justified row bottom gap
	const HEADER_H = 44;
	 ...
	 *
	 * Video thumbnails are produced server-side (ffmpeg) on upload through the
	 * same WebP pipeline as photos — the browser never re-downloads the video
	 * to extract a frame.
	 */
...
<div
	class="sticky top-0 z-20 flex flex-wrap items-center justify-between gap-3 bg-base-100 px-4 pt-4"
>
	<h1 class="m-0 text-2xl font-bold text-base-content">{t('nav.photos', 'Photos')}</h1>
	<div class="tabs tabs-border" role="tablist" aria-label={t('nav.photos', 'Photos')}>
		<button
			class="tab {tab === 'moments' ? 'tab-active text-primary' : ''}"
			role="tab"
		...
		<button
			class="tab {tab === 'places' ? 'tab-active text-primary' : ''}"
			role="tab"
		...
		{#if peopleAvailable}
			<button
				class="tab {tab === 'people' ? 'tab-active text-primary' : ''}"
				role="tab"
...
{#if tab === 'moments'}
	<div class="flex items-center justify-between gap-3 px-4 pt-3">
		<div class="join" role="group" aria-label={t('photos.group_by', 'Group by')}>
			{#each MODES as m (m)}
				<button
					class="btn join-item btn-sm capitalize {groupMode === m ? 'btn-primary' : ''}"
		...
		<div class="join" role="group" aria-label={t('photos.layout_square', 'Layout')}>
			<button
				class="btn btn-square join-item btn-sm {layoutMode === 'square' ? 'btn-primary' : ''}"
			...
			>
			<button
				class="btn btn-square join-item btn-sm {layoutMode === 'justified' ? 'btn-primary' : ''}"
	...
	{#if selected.size > 0}
		<div
			class="mx-4 mt-3 flex items-center justify-between gap-3 rounded-lg border border-base-300 bg-primary/10 px-3 py-2"
			data-testid="photos-batch-bar"
		>
			<span>{t('files.selected_count', { count: selected.size }, '{{count}} selected')}</span>
			<div class="flex gap-2">
				<Button small data-testid="photos-batch-download-btn" onclick={downloadSelected}
					>{t('common.download', 'Download')}</Button
				>
				<Button small data-testid="photos-batch-clear-btn" onclick={() => selected.clear()}
					>{t('common.clear', 'Clear')}</Button
				>
				<Button small data-testid="photos-batch-delete-btn" variant="danger" onclick={trashSelected}
	...
	{#if error}
		<p class="py-8 text-center text-error" role="alert">{error}</p>
			...
			<div bind:clientWidth={gridWidth}>
				{#if photoRows.length}
					<VirtualRows rows={photoRows} overscan={1000}>
						{#snippet row(r)}
							{#if r.kind === 'header'}
								<!-- Date header — fixed height (set inline) so the virtualizer's
								     offset table matches the rendered layout exactly. -->
								<div
									class="flex items-center gap-2 text-base text-base-content"
									style:height="{r.height}px"
								>
									{r.label} <span class="text-sm text-base-content/60">{r.count}</span>
								</div>
							{:else}
								<!-- A horizontal strip of explicitly-sized tiles — one virtualized
								     row, used by both the square and justified layouts (the bottom
								     gap is baked into the row's declared height). -->
								<div class="flex" style:height="{r.height}px" style:gap="{r.gap}px">
	...
	{#if loading}<p class="py-8 text-center text-base-content/60">
...
{#snippet tile(photo: PhotoItem, sizeStyle?: string)}
	<div
		class="group relative overflow-hidden rounded bg-base-300 {selected.has(photo.id)
	...
	>
		...
		>
			...
			<span
				class="pointer-events-none absolute inset-0 grid place-items-center text-3xl text-base-content/30"
				aria-hidden="true"><Icon name="file-image" /></span
			>
			<img
			...
			{#if isVideo(photo)}
				<span
					class="pointer-events-none absolute bottom-1.5 right-1.5 grid h-[26px] w-[26px] place-items-center rounded-full bg-neutral/60 text-[0.7rem] text-neutral-content"
					aria-hidden="true"><Icon name="play" /></span
				>
			{/if}
		</button>
		<button
			class="absolute left-1.5 top-1.5 grid h-6 w-6 cursor-pointer place-items-center rounded-full border-2 transition-opacity group-hover:opacity-100 {selected.has(
				photo.id
			)
				? 'border-primary bg-primary text-primary-content opacity-100'
				: 'border-neutral-content bg-neutral/60 text-transparent opacity-0'}"
