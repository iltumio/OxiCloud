<script lang="ts">
	...
	async function uploadAll(
		...
		const frac = Array.from({ length: total }, () => 0);
		let savedBytes = 0;
		let failures = 0;
		let next = 0;
	...
	async function collectDroppedEntries(
		...
		async function walk(entry: FileSystemEntry, prefix: string): Promise<void> {
			if (entry.isFile) {
				const file = await new Promise<File>((res, reject) =>
					(entry as FileSystemFileEntry).file(res, reject)
				);
				...
				for (;;) {
					const batch = await new Promise<FileSystemEntry[]>((res, reject) =>
						reader.readEntries(res, reject)
					);
	 ...
	 * a count badge (ported from ui.js). The `.drag-preview` / `.dragged-items` /
	 * `.dragged-items-badge` styles live in this page's style block (`:global`,
	 * because the ghost is appended to the document body, outside this subtree).
	 */
	...
	const sortedFolders = $derived(listing.folders.toSorted(cmpFolders));
	const sortedFiles = $derived(listing.files.toSorted(cmpFiles));
	...
	/** List wrapper — hosts the column track var read by the header + rows. */
	const LIST_WRAP_CLASS = 'bg-base-100 flex w-full flex-col overflow-hidden rounded-2xl shadow-sm';
	/** checkbox · name · owner · type · size · date · actions (legacy track). */
	const LIST_COLUMNS = '36px minmax(200px, 2fr) 120px 100px 110px 130px 200px';
	/** List-only columns collapse on phones (icon + name … size + actions stay). */
	const listCell = 'hidden min-w-0 truncate min-[641px]:block';

	// Row shell per view mode. List rows: compact flex line on phones, column
	// grid from 641px (mirrors the legacy 640px reflow breakpoint).
	const rowShell = $derived(
		isGrid
			? 'group relative flex min-h-40 w-full cursor-pointer flex-col items-center rounded-2xl border border-base-300 bg-base-100 p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-primary hover:shadow-lg'
			: 'group flex cursor-pointer items-center gap-3 border-b border-base-300/40 px-4 py-2 transition-[background-color,box-shadow] hover:bg-base-200/60 hover:shadow-[inset_3px_0_0_var(--color-primary)] min-[641px]:grid min-[641px]:gap-x-3 min-[641px]:[grid-template-columns:var(--files-list-columns)]'
	);
	function rowClass(id: string, isDropTarget = false): string {
		const selectedCls = selected.has(id)
			? isGrid
		...
		const dropCls = isDropTarget
			? isGrid
				...
				: 'bg-warning/10 outline-2 -outline-offset-2 outline-dashed outline-warning'
			: '';
		return `${rowShell} ${selectedCls} ${dropCls}`;
	}

	/** Coarse file-kind → daisyUI semantic colour for the type-tinted icon tile
	 *  (same per-surface mapping ResourceList uses for its rows). */
	...
	function iconTileClass(iconName: string): string {
		const color = KIND_COLOR[fileIconKind(iconName)] ?? KIND_COLOR.generic;
		return `${color} relative flex shrink-0 items-center justify-center overflow-hidden bg-current/10 ring-1 ring-current/20 ring-inset ${
			isGrid ? 'mb-3 aspect-[4/3] w-full rounded-xl text-4xl' : 'h-10 w-10 rounded-xl text-base'
		}`;
	}

	// Cell / control classes shared by the folder and file row snippets.
	const nameCellClass = $derived(
		isGrid
			? 'w-full min-w-0 text-center'
			: 'flex min-w-0 flex-1 items-center gap-3 overflow-hidden min-[641px]:flex-none'
	);
	const nameTextClass = $derived(
		isGrid
			? 'block w-full max-w-full truncate py-1 text-center text-sm font-medium text-base-content'
			: 'min-w-0 truncate text-base-content'
	);
	const checkCellClass = $derived(
		isGrid
			? 'absolute top-5 left-5 z-10 flex items-center justify-center opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100'
			: 'flex shrink-0 items-center justify-center'
	);
	const actionCellClass = $derived(
		isGrid ? 'contents' : 'flex shrink-0 items-center justify-end gap-0.5'
	);
	const btnAction =
		'btn btn-ghost btn-xs btn-square text-base-content/60 no-underline hover:text-base-content';
	function favStarClass(active: boolean): string {
		if (isGrid) {
			return `btn btn-circle btn-xs bg-base-100/80 absolute top-5 right-[54px] z-[12] border-none shadow-sm backdrop-blur-sm ${
				active
					? 'text-warning'
					: 'text-base-content/60 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100'
			}`;
		}
		return `btn btn-ghost btn-xs btn-square ${
			active
				? 'text-warning'
				: 'text-base-content/40 invisible group-hover:visible group-focus-within:visible'
		}`;
	}
	// Grid: corner kebab on a frosted scrim; list: fades in on hover-capable
	// devices, always tappable on touch (no @media(hover) → visible).
	const kebabClass = $derived(
		isGrid
			? 'btn btn-circle btn-xs bg-base-100/80 text-base-content/70 absolute top-5 right-5 z-10 border-none shadow-sm backdrop-blur-sm opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100'
			: 'btn btn-ghost btn-xs btn-square text-base-content/60 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 [@media(hover:hover)]:opacity-0'
	);
	// Shared badge: inline chip in list rows, corner scrim badge on grid cards.
	const sharedBadgeClass = $derived(
		isGrid
			? 'bg-base-100/80 text-base-content/70 absolute top-5 left-5 z-[9] flex h-6 w-6 items-center justify-center rounded-full text-xs shadow-sm backdrop-blur-sm'
			: 'text-base-content/50 ml-1 inline-flex shrink-0 items-center text-xs'
	);
	const thumbClass = $derived(
		`pointer-events-none absolute inset-0 z-[1] h-full w-full object-cover ${
			isGrid ? 'transition-transform duration-300 group-hover:scale-[1.04]' : ''
		}`
	);
	const ctxItemClass =
		'flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-base-content no-underline hover:bg-base-200';
	...
	// Close the upload popup when clicking outside of it. `.upload-dropdown` is
	// an unstyled behaviour hook, not a styling class.
	$effect(() => {
...
<div
	class="min-h-full {dragOver
		? 'rounded-2xl outline-2 -outline-offset-8 outline-dashed outline-primary'
		: ''}"
	role="region"
...
>
	<div class="flex flex-col gap-3">
		...
		>
			{#snippet start()}
				{#if selectedCount > 0}
					<div
						class="bg-primary/10 border-base-300 flex min-w-0 flex-wrap items-center gap-2 rounded-xl border px-3 py-1.5"
						data-testid="files-batch-bar"
					>
						<button
						...
						<span class="text-base-content font-semibold whitespace-nowrap"
							>{t('files.selected_count', { count: selectedCount }, '{{count}} selected')}</span
						>
						<div class="flex items-center gap-1">
							<button
								class="btn btn-ghost btn-sm gap-1.5 normal-case"
								title={t('files.add_favorites', 'Add to favorites')}
								data-testid="files-batch-favorite-btn"
								onclick={() => void batchFavorites()}
							>
								<Icon name="star" />
								<span class="hidden min-[641px]:inline"
									>{t('files.add_favorites', 'Add to favorites')}</span
								>
							</button>
							<button
								class="btn btn-ghost btn-sm gap-1.5 normal-case"
							...
							>
								...
								<span class="hidden min-[641px]:inline">{t('files.move', 'Move')}</span>
							</button>
							<button
								class="btn btn-ghost btn-sm gap-1.5 normal-case"
							...
							>
								<Icon name="copy" />
								<span class="hidden min-[641px]:inline">{t('files.copy', 'Copy')}</span>
							</button>
							<button
								class="btn btn-ghost btn-sm gap-1.5 normal-case"
							...
							>
								<Icon name="download" />
								<span class="hidden min-[641px]:inline">{t('common.download', 'Download')}</span>
							</button>
							<button
								class="btn btn-ghost btn-sm text-error gap-1.5 normal-case"
								title={t('common.delete', 'Delete')}
								data-testid="files-batch-delete-btn"
								onclick={batchDelete}
							>
								<Icon name="trash" />
								<span class="hidden min-[641px]:inline">{t('common.delete', 'Delete')}</span>
							</button>
						</div>
					</div>
				{:else}
					<div class="flex items-center gap-2">
						<div class="upload-dropdown relative" data-testid="files-upload-dropdown">
							<button
							...
							>
								...
								>
								<Icon name="caret-down" class="text-xs opacity-80" />
							</button>
							{#if uploadMenuOpen}
								<div
									class="bg-base-100 border-base-300 rounded-box absolute top-full left-0 z-20 mt-1 flex w-56 flex-col border p-1 shadow-lg"
									data-testid="files-upload-menu"
								>
									<button
						...
						<button class="btn" data-testid="files-new-folder-btn" onclick={onNewFolder}>
		...
		<nav class="breadcrumbs px-1 py-0 text-sm" aria-label="Breadcrumb">
			<ul>
				...
				<li>
					<a
						href={resolve('/files')}
						class="text-base-content/60 hover:text-primary rounded-md px-1 py-0.5"
				...
				{#each crumbs as c, i (c.id)}
					<li>
						{#if i === crumbs.length - 1}
							<span class="text-base-content px-1 py-0.5 font-semibold">{c.name}</span>
						{:else}
							<a
	...
	{:else}
		<div class="relative" bind:clientWidth={gridWidth}>
			{#if groupBy !== ''}
				<div
					class={isGrid ? GRID_CLASS : LIST_WRAP_CLASS}
					style="--files-list-columns: {LIST_COLUMNS}"
				>
					{#if !isGrid}{@render fileListHeader()}{/if}
					{#each groups as group (group.key)}
						<div
							class="border-base-300/60 text-base-content/50 col-span-full border-b px-2 pt-3 pb-1 text-xs font-semibold tracking-wider uppercase select-none"
							role="rowheader"
						>
							{group.label}
						</div>
			...
			{:else if !isGrid}
				...
				<div class={LIST_WRAP_CLASS} style="--files-list-columns: {LIST_COLUMNS}">
...
{#snippet fileListHeader()}
	<!-- Phones collapse rows to a flex line, so the column header only appears
	     from 641px (mirrors the legacy 640px reflow). -->
	<div
		class="border-base-300 bg-base-200 text-base-content hidden items-center gap-x-3 border-b px-4 py-3.5 text-sm font-semibold min-[641px]:grid min-[641px]:[grid-template-columns:var(--files-list-columns)]"
	>
		...
		{#each [{ f: 'name', l: t('files.col_name', 'Name') }, { f: 'owner', l: t('files.col_owner', 'Owner') }, { f: 'type', l: t('files.col_type', 'Type') }, { f: 'size', l: t('files.col_size', 'Size') }, { f: 'modified_at', l: t('files.col_modified', 'Modified') }] as col (col.f)}
			{#if col.f === 'owner'}
				<div class="min-w-0 truncate">{col.l}</div>
			{:else}
				<button
					class="hover:text-primary flex cursor-pointer items-center gap-1 text-left font-semibold {sortField ===
					col.f
						? 'text-primary'
						: ''} {col.f === 'size' ? 'justify-self-end text-right' : ''}"
				...
				>
					{col.l}
					{#if sortField === col.f}
						<Icon name={sortDir === 1 ? 'arrow-down' : 'arrow-up'} class="text-xs" />
					{/if}
				</button>
			{/if}
		{/each}
...
{#snippet folderRow(folder: FolderItem)}
	<div
		class={rowClass(folder.id, dropFolderId === folder.id)}
		role="button"
		tabindex="0"
		draggable="true"
	...
	>
		<div class="{checkCellClass} {selected.has(folder.id) ? 'opacity-100' : ''}">
			<input
				type="checkbox"
		...
		<div class={nameCellClass}>
			<div class={iconTileClass('folder')}><Icon name="folder" /></div>
			<span class={nameTextClass} title={folder.name}>{folder.name}</span>
			{#if !isGrid && favoriteIds.has(folder.id)}
				<div
					class="text-warning ml-1 inline-flex shrink-0 items-center text-xs"
			...
			{#if sharedIds.has(folder.id)}
				<div class={sharedBadgeClass} title={t('files.shared', 'Shared')}>
					<Icon name="oxiexport" />
				</div>
			{/if}
		</div>
		{#if isGrid}
			<!-- Grid caption: recency under the tile (grid cards have no columns). -->
			<div
				class="text-base-content/60 mt-0.5 flex max-w-full items-center justify-center gap-1 text-[11px]"
			>
				<span class="min-w-0 truncate">{relativeTimeAgo(folder.modified_at)}</span>
			</div>
		{:else}
			<div class="{listCell} text-base-content/70">
				{ownerLabel(folder.created_by, session.user?.id ?? null)}
			</div>
			<div class="{listCell} text-base-content/70">{t('files.file_types.folder', 'Folder')}</div>
			<div
				class="text-base-content/50 min-w-0 shrink-0 truncate text-right tabular-nums min-[641px]:justify-self-end"
			>
				—
			</div>
			<div class="{listCell} text-base-content/70">{formatDate(folder.modified_at)}</div>
		{/if}
		<div class={actionCellClass}>
			<button
				class={favStarClass(favoriteIds.has(folder.id))}
			...
			{#if !isGrid}
				...
				>
				<button
					class="{btnAction} hover:text-error"
					title={t('common.delete', 'Delete')}
...
{#snippet fileRow(file: FileItem)}
	...
	>
		<div class="{checkCellClass} {selected.has(file.id) ? 'opacity-100' : ''}">
			<input
				type="checkbox"
				class="checkbox checkbox-sm checkbox-primary bg-base-100"
		...
		<div class={nameCellClass}>
			<div class={iconTileClass(iconName)}>
			...
			<span class={nameTextClass} title={file.name}>{file.name}</span>
			{#if !isGrid && favoriteIds.has(file.id)}
				<div
					class="text-warning ml-1 inline-flex shrink-0 items-center text-xs"
					title={t('files.favorited', 'Favorite')}
				>
		...
		{#if isGrid}
			<!-- Grid caption: recency + size ("hace 2 días · 4,31 MB"). -->
			<div
				class="text-base-content/60 mt-0.5 flex max-w-full items-center justify-center gap-1 text-[11px]"
			>
				<span class="min-w-0 truncate">{relativeTimeAgo(file.modified_at)}</span>
				{#if file.size != null}<span class="text-base-content/40 shrink-0"
		...
		{:else}
			...
			<div class="{listCell} text-base-content/70">{typeLabel(file.category)}</div>
			<div
				class="text-base-content/70 min-w-0 shrink-0 truncate text-right tabular-nums min-[641px]:justify-self-end"
			>
				{file.size != null ? formatBytes(file.size) : ''}
			</div>
			<div class="{listCell} text-base-content/70">{formatDate(file.modified_at)}</div>
		{/if}
		<div class={actionCellClass}>
			<button
				class={favStarClass(favoriteIds.has(file.id))}
...
{#if ctxOpen && ctxTarget}
	...
	></div>
	<div
		class="bg-base-100 border-base-300 rounded-box fixed z-50 flex min-w-[200px] flex-col border p-1 shadow-lg"
		style:left="{ctxX}px"
		style:top="{ctxY}px"
		role="menu"
...
<style>
	/* Multi-item drag ghost — built imperatively in showDragGhost() and appended
	   to <body>, so it lives outside this component's subtree (`:global`).
	   Utilities can't reach dynamically-created nodes; colours stay on daisyUI
	   theme variables. */
	:global(.drag-preview) {
		position: absolute;
		top: -9999px;
	...
	:global(.drag-preview .dragged-items) {
		display: flex;
	...
	:global(.drag-preview .dragged-items .file-item) {
		display: flex;
		align-items: center;
		gap: 0.75rem;
		width: 360px;
	...
	:global(.drag-preview .dragged-items .file-item div:nth-child(2)) {
		overflow: hidden;
		white-space: nowrap;
		text-overflow: ellipsis;
	}

	:global(.drag-preview .dragged-items .file-item .file-icon) {
		display: flex;
		align-items: center;
	...
	:global(.drag-preview .dragged-items .fading) {
		mask-image: linear-gradient(to bottom, var(--color-base-100) 10%, transparent);
	}

	:global(.drag-preview .dragged-items-badge) {
		position: absolute;
		top: 0;
		right: 0;
